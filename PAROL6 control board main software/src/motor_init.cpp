...
void Init_Joint_1(struct MotorStruct *Joint__)
{
    ...
    Joint__->LIMIT = LIMIT6;                    // Пин подключения концевого выключателя (определен в iodefs.h)
    Joint__->DIR = DIR1;                        // Пин направления вращения двигателя
    Joint__->STEP = PUL1;                       // Пин шага (импульса) двигателя
    Joint__->SELECT = SELECT1;                  // Пин выбора (CS) драйвера двигателя на шине SPI
